...
const Test = ({ route, navigation }) => {
    ...
    const [selectorButton, setSelectorButton] = useState(0);
    ...
    const address = `${CITY_TYPE_MAPPING[inputData.city] || ''} ${inputData.location || ''}`.trim();
    const staticMapUrl = `https://maps.googleapis.com/maps/api/staticmap?center=${encodeURIComponent(address)}&zoom=13&size=500x100&markers=color:red%7C${encodeURIComponent(address)}&key=${GOOGLE_MAPS_API_KEY}`;
        ...
        <View style={styles.container}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
                ...
                <View style={styles.topPortionContainer}>
                    ...
                    <Text style={styles.date}>{`${inputData.date} ${inputData.start_time?.slice(0, 5)} - ${inputData.is_overnight ? "（隔天）": ""}${inputData.end_time?.slice(0, 5)}`}</Text>
                    <View style={styles.creatorRow}>
                        <Image 
                            style={styles.creatorImage}
                            source={{uri: "https://accademiagallery.org/wp-content/uploads/2024/08/Michelangelos-Statue-of-David-in-florence-accademia-gallery-1.jpg"}}
                        />
                        <Text style={[styles.creatorName]}>{inputData.created_by_nickname}</Text>
                ...
                (
                    <>
                        <View style={styles.contentSelector}>
                            <TouchableOpacity 
                                style={selectorButton == 0 ? styles.selectorButtonTrue : styles.selectorButtonFalse}
                                onPress={() => setSelectorButton(0)}
                            >
                                <Text style={selectorButton == 0 ? styles.selectorTextTrue : styles.selectorTextFalse}>報名資訊</Text>
                            </TouchableOpacity>
                            <TouchableOpacity 
                                style={selectorButton != 1 ? styles.selectorButtonFalse : styles.selectorButtonTrue}
                                onPress={() => setSelectorButton(1)}
                            >
                                <Text style={selectorButton != 1 ? styles.selectorTextFalse : styles.selectorTextTrue}>審核資訊</Text>
                            </TouchableOpacity>
                            {inputData.is_creator && (
                                <TouchableOpacity 
                                    style={selectorButton != 2 ? styles.selectorButtonFalse : styles.selectorButtonTrue}
                                    onPress={() => setSelectorButton(2)}
                                >
                                    <Text style={selectorButton != 2 ? styles.selectorTextFalse : styles.selectorTextTrue}>取消報名</Text>
                                </TouchableOpacity>
                            )}
                        </View>
                        ...
                        {selectorButton == 0 && (
                            <>
                                ...
                                <Image source={require('../../../assets/images/location_pin.png')} style={styles.locationIcon} />
                                <Text style={styles.addressText}>地址：{CITY_TYPE_MAPPING[inputData.city] || ""} {inputData.location}</Text>
                                </View>
                                    ...
                                    {address ? (
                                        <TouchableOpacity onPress={openGoogleMaps}>
                                            <Image
                                                ...
                                                onError={(error) => console.error('Static map error:', error.nativeEvent)}
                                                accessibilityLabel="Static Map"
                                                accessibilityHint="Tap to open the location in Google Maps"
                                            />
                                        </TouchableOpacity>
                                    ) : (
                                        <Text style={styles.noMapText}>無可用地址</Text>
                                    )}
                                </View>

                                <View style={styles.informationRow}>
                                <View style={styles.informationColumn}>
                                    <Text style={styles.informationTitle}>網子類型</Text>
                                    <View style={styles.informationBox}>
                                    <Text style={styles.informationContent}>{NET_TYPE_MAPPING[inputData.net_type] || inputData.net_type}</Text>
                                    </View>
                                </View>
                                <View style={styles.informationColumn}>
                                    <Text style={styles.informationTitle}>場次等級</Text>
                                    <View style={styles.informationBox}>
                                    <Text style={styles.informationContent}>{inputData.level}</Text>
                                    </View>
                                </View>
                                <View style={styles.informationColumn}>
                                    <Text style={styles.informationTitle}>參加人數</Text>
                                    <View style={styles.informationBox}>
                                    <Text style={styles.informationContent}>{inputData.spots_left}</Text>
                                    </View>
                                </View>
                                <View style={styles.informationColumn}>
                                    <Text style={styles.informationTitle}>費用</Text>
                                    <View style={styles.informationBox}>
                                    <Text style={styles.informationContent}>${inputData.cost.slice(0, -3)}</Text>
                                    </View>
                                </View>
                                </View>

                                <View style={styles.participantContainer}> 
                                <Text style={styles.participantText}>參與者：</Text>
                                </View>
                                ...
                                <View style={styles.participantImageContainer}>
                                    ...
                                    />
                                    <TouchableOpacity 
                                        style={styles.moreParticipant}
                                        onPress={() => navigation.navigate("MoreParticipant", { users: approved })}
                                    >
                                        <Text style={styles.moreParticipantText}>•••</Text>
                                    </TouchableOpacity>
                                </View>

                                <View style={styles.notesContainer}>
                                <Text style={styles.notesTitleText}>活動備註：</Text>
                                <Text style={styles.additionalNotesText} selectable={true} selectionColor='orange'>{inputData.additional_comments}</Text>
                                </View>
                            </>
                        )}

                        
                        {inputData.is_creator & selectorButton == 1 ? (
                                ...
                                {pending.map((pendingUser, index) => (
                                    <View key={index} style={styles.pendingUserContainer}>
                                        <Image 
                                            ...
                                            source={{uri: "https://accademiagallery.org/wp-content/uploads/2024/08/Michelangelos-Statue-of-David-in-florence-accademia-gallery-1.jpg"}}
                                        />
                                        <View style={styles.participantPendingColumnContainer}>
                                            <View style={styles.participantPendingInfoContainer}>
                                                <Text style={[styles.pendingUserText, styles.pendingUserName]}>{pendingUser.user_nickname}</Text>
                                                <View style={styles.participantLevelTag}>
                                                    <Text style={styles.participantLevelText}>D</Text>
                                                </View>
                                                ...
                                                <Text style={styles.createdTimeText}>{pendingUser.created_at}</Text>
                                            </View>
                                            <View style={styles.additionalNotesContainer}>
                                                <Text style={styles.additionalRegisterNotesText}>{pendingUser.notes}</Text>
                                            </View>
                                        </View>
                                        <View style={styles.registerPeopleContainer}>
                                            <Text style={[styles.registerPeopleText]}>{pendingUser.number_of_people}</Text>
                                        </View>    
                                ...
                                <View style={{paddingHorizontal: 20}}>
                                    ...
                                    {approved.map((approvedUser, index) => (
                                        <View key={index} style={styles.pendingUserContainer}>
                                            ...
                                            <View style={styles.participantPendingColumnContainer}>
                                                <View style={styles.participantPendingInfoContainer}>
                                                    <Text style={[styles.pendingUserText, styles.pendingUserName]}>{approvedUser.user_nickname}</Text>
                                                    <View style={styles.participantLevelTag}>
                                                        <Text style={styles.participantLevelText}>D</Text>
                                                    </View>
                                                    ...
                                                    <Text style={styles.createdTimeText}>{approvedUser.created_at}</Text>
                                                </View>
                                                <View style={styles.additionalNotesContainer}>
                                                    <Text style={styles.additionalRegisterNotesText}>{approvedUser.notes}</Text>
                                                </View>
                                            </View>
                                            <View style={styles.registerPeopleContainer}>
                                                <Text style={[styles.registerPeopleText]}>{approvedUser.number_of_people}</Text>
                                            </View>    
                                            {inputData.is_creator && (
                                ...
                                <View style={{paddingHorizontal: 20}}>
                                    ...
                                    {approved.map((approvedUser, index) => (
                                        <View key={index} style={styles.pendingUserContainer}>
                                            ...
                                            {inputData.is_creator && (
                                                <TouchableOpacity style={styles.kickButton} onPress={() => handleKickPressed(approvedUser.user_id)}>
                                                    <Text style={styles.verifyButtonText}>同意</Text>
                                                </TouchableOpacity>
                                            )}
                                        </View>
                                    ))}
...
const styles = StyleSheet.create({
    ...
    participantPendingColumnContainer: {
        flexDirection: "column",
        alignItems: "flex-start",
        
    },
    participantPendingInfoContainer: {
        flexDirection: "row",
        alignItems: "flex-end",
        paddingHorizontal: 5
    },
    ...
    additionalRegisterNotesText: {
        color: "black",
        fontSize: 11,
        fontWeight: "600"
    },
    registerPeopleContainer: {
        height: 40,
        width: 40, 
        borderRadius: 5,
        marginLeft: 15,
    ...
    staticMap: {
        width: "100%", // Adjust for paddingHorizontal: 20
        height: "100%",
        borderRadius: 10,
    },
    noMapText: {
